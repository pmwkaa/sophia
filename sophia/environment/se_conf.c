...
static inline int
se_confdb_deadlock(srconf *c, srconfstmt *s)
{
	if (s->op != SR_WRITE)
		return se_confv(c, s);
...
se_confdb(se *e, seconfrt *rt ssunused, srconf **pc)
{
	...
	ss_listforeach(&e->db.list, i)
	{
		...
		sr_C(&p, pc, se_confv, "node_size", SS_U64, &o->rtp.total_node_size, SR_RO, NULL);
		sr_C(&p, pc, se_confv, "node_origin_size", SS_U64, &o->rtp.total_node_origin_size, SR_RO, NULL);
		...
		sr_c(&p, pc, se_confdb_deadlock, "deadlock", SS_FUNCTION, NULL);
