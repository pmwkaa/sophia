...
static int
update_operator_orphan(void *src, int srcsize, void *update, int update_size, void *arg,
                       void **result, int *resultsize)
{
	assert(src == NULL);
	assert(srcsize == 0);
	(void)arg;
	char *s = malloc(update_size);
	memcpy(s, update, update_size);
	*result = s;
	*resultsize = update_size;
	update_ops++;
	return 0;
}
