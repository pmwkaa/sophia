...
leak_set_get(void)
{
	...
	t( sp_getint(env, "memory.v_count") == 1 );
...
leak_tx_tx_set_get_commit0(void)
{
	...
	t( sp_getint(env, "memory.v_count") == 2 );
	t( sp_commit(b) == 0 );
	t( sp_getint(env, "memory.v_count") == 1 ); /* last read freed */
...
leak_tx_tx_tx_set_get_commit0(void)
{
	...
	t( sp_getint(env, "memory.v_count") == 3 );
	t( sp_commit(c) == 0 );
	t( sp_getint(env, "memory.v_count") == 3 );
...
stgroup *leak_group(void)
{
	...
	st_groupadd(group, st_test("batch_commit", leak_batch_commit));
	st_groupadd(group, st_test("batch_rollback", leak_batch_rollback));
